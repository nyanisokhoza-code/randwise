...
function openAddGoalPWA(){
  ...
  var gt = document.getElementById('goal-target');
  var gs = document.getElementById('goal-saved');
  var gd = document.getElementById('goal-date');
  var gr = document.getElementById('goal-calc-result');
  if(gn) gn.value = '';
  if(gt) gt.value = '';
  if(gs) gs.value = '';
  if(gd) gd.value = '';
...
function calcGoalMonthly(){
  ...
  var savedEl = document.getElementById('goal-saved');
  var dateEl = document.getElementById('goal-date');
  var resultEl = document.getElementById('goal-calc-result');
  if(!resultEl || !targetEl) return;
  var target = parseFloat(targetEl.value || 0);
  var saved = parseFloat(savedEl ? savedEl.value : 0) || 0;
  var dateStr = dateEl ? dateEl.value : '';
  if(!target || !dateStr){ resultEl.textContent = ''; return; }
  var remaining = target - saved;
  if(remaining <= 0){ resultEl.textContent = 'Goal already reached!'; return; }
  var targetDate = new Date(dateStr);
  var today = new Date();
  var months = Math.max(1, Math.ceil((targetDate - today) / (1000 * 60 * 60 * 24 * 30)));
  var monthly = remaining / months;
  resultEl.textContent = 'Save R' + monthly.toFixed(0) + '/month for ' + months + ' months';
}

function saveGoalPWA(){
  ...
  if(!name || !target || !dateStr){
    if(typeof toast === 'function') toast('Please fill in goal name, target amount and target date');
    return;
  }
  var goal = {
    tester_id: (typeof currentUser !== 'undefined' && currentUser) ? currentUser.id : null,
    name: name, emoji: 'goal', target: target, saved: saved || 0,
    target_date: dateStr, monthly_contribution: 0, account_nickname: ''
  };
  if(typeof sbPost === 'function'){
    sbPost('savings_goals', goal).then(function(res){
      if(res.error){ console.error('saveGoalPWA error:', res.error); if(typeof toast === 'function') toast('Failed to save goal.'); }
      else { if(typeof toast === 'function') toast('Goal saved!'); closeAddGoalPWA(); if(typeof loadGoalsPWA === 'function') loadGoalsPWA(); }
    });
  }
}
